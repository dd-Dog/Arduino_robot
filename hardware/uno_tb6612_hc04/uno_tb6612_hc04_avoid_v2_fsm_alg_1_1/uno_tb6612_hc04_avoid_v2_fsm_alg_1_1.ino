 ...
 *
 * ALG_ID = "ALG-1.1"  slug: fsm_rhr
 * 与 ALG-1.0 的差异见本平台 VERSION.md / docs/DEVELOPMENT.md
 *
 * 主要变化：
...
#define DEBUG_SENSOR_ONLY  0   // 1=状态机+SERIAL_DEBUG 照常，电机不转（测完改回 0）
...
static byte blockedStreak = 0;          // 连续 d<dBrake，满 BLOCK_CONFIRM 进 DECIDE
static byte clearStreak = 0;           // 连续 d>=dProbeOk，满 CLEAR_CONFIRM 才回 FORWARD
static byte nearStreak = 0;            // 连续 d<dStop，满 NEAR_CONFIRM 才 BACKUP
...
void setup() {
  ...
  Serial.begin(9600);
  delay(200);
  Serial.print(F("uno_tb6612_hc04_avoid_v2_fsm_alg_1_1 "));
  Serial.print(ALG_ID);
  Serial.print(F(" "));
...
// DECIDE 判前方够宽；allowRightAfterBackup=1 时探测转完后可回 FORWARD（冷却内仍禁止「未转就直行」）
bool decideTryForward(long cm, bool allowRightAfterBackup) {
  if (cm <= 0 || cm < dProbeOk()) {
    clearStreak = 0;
    return false;
...
void runForward() {
  ...
  // 极近：连续确认后再后退；刚退过后冷却期内不再 BACKUP，改 DECIDE
  if (d > 0 && d < dStop()) {
    if (backupInCooldown()) {
      nearStreak = 0;
      blockedStreak++;
...
void runDecide() {
  ...
  switch (probePhase) {
    case 0: {
      // 未转过向前够宽 → 连续确认后才 FORWARD；刚 BACKUP 后冷却内禁止此捷径
