...
演示版已在实车调试通过（前进 / 后退 / 左转 / 右转）；避障版含 HC-SR04、舵机扇区扫描与自适应脱困，已实车可跑。后续规划见 [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md)。
...
```
Arduino_robot/                 ← Cursor 建议打开此仓库根目录
├── README.md
├── docs/
│   └── DEVELOPMENT.md         ← 分阶段开发规划（状态机 / 右手法则 / 编码器等）
├── car_tb6612/
│   └── car_tb6612.ino         ← 演示版（动作循环，随时可刷回）
└── car_tb6612_avoid/
    └── car_tb6612_avoid.ino   ← 避障版（HC-SR04 + 舵机扫描 + 自适应脱困）
```
...
| HC-SR04（或 HC-SR04+） | 1 | 仅避障版；装在舵机云台上；Trig/Echo 脉冲模式 |
| SG90 舵机 | 1 | 仅避障版；信号 **D11**，中位 90° 朝前 |
...
- **舵机**：固定在车头中部；**信号线 → D11**；5V/GND 与系统共地（电流大时可舵机单独 5V 供电）。
- **超声波**：装在舵机臂上，**中位（约 90°）时**两圆孔**水平朝前**（勿朝下照地、勿朝上）。
...
| 舵机线 | Arduino | 说明 |
|--------|---------|------|
| 信号（橙/黄） | **D11** | `Servo` 库，中位 90° |
| Vcc（红） | **5V** | 可与电源模块 5V 共轨 |
| Gnd（棕/黑） | **GND** | 必须共地 |
...
| **舵机扇区扫描** | 遇障停稳后 5 角度扫描（约 55°~125°），选较空一侧转向；同距优先右转 |
| 遇障后退 + 转向 | 低速前进，近障触发 |
| 自适应脱困 | 短时间内再次遇障 → `stuckLevel` 升高；后退/转向时间加大；**有舵机时按扫描选向**，无舵机时与上次反向；畅通 2.5s 复位 |
| 串口调试 | `SERIAL_DEBUG`、`SERIAL_DEBUG_RAW`（9600） |
| 仅测距模式 | `DEBUG_SENSOR_ONLY=1` |
| 仅扫描模式 | `DEBUG_SCAN_ONLY=1`（电机不动，看各角度 `cm`） |
| 无舵机回退 | `USE_SERVO_SCAN=0` 恢复左右交替转向 |
...
舵机扫描可判断左右哪侧更空；**死胡同仍可能出不来**。无舵机时（`USE_SERVO_SCAN=0`）仍为左右交替。
...
| 功能 | 状态 |
|------|------|
| 状态机（FORWARD / SCAN / TURN / …） | 📋 已规划 |
| 转向避障记录 | 📋 已规划 |
| 右手法则（优先右转→直行→左转→后退） | 📋 已规划 |
| 编码器定角转向 | 📋 已规划 |
| 按键 / 遥控 | ❌ |
| 串口控制 | ❌ |
| 循迹 | ❌ |
...
1. IDE 打开 **`car_tb6612_avoid`**，接好 TB6612 + HC-SR04（Trig=D9，Echo=D12）+ 舵机（**D11**）。
2. 先 `DEBUG_SCAN_ONLY=1`，串口 **9600** 看 5 个角度 `cm` 是否随舵机转向变化。
3. 再 `DEBUG_SENSOR_ONLY=1` 看中位朝前 `cm=` 是否正常。
4. 全部改 `0` 上传；架空再地面试避障。
5. 该前进却倒车：改 `MOTOR_*_DIR_REVERSE`（本仓库避障版为 `true`）。
6. 调参：窄道仍摆 → 加大 `MS_TURN_STEP` 或 `STUCK_REPEAT_MS`；误触发 → 减小 `OBSTACLE_CM_MIN`；扫描太慢 → 减小 `SERVO_SETTLE_MS`（可能不稳）。
...
| 窄道左右摆不出 | 升档参数或加大扫描角范围；见 DEVELOPMENT.md 后续阶段 |
| 舵机不动 / Uno 复位 | 舵机电流过大；舵机 5V 单独供电、共地 |
...
- 舵机扫描：HC-SR04 上云台、D11 舵机、遇障扇区扫描选向；`docs/DEVELOPMENT.md` 记录状态机 / 右手法则 / 编码器等后续阶段。
